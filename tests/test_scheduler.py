...
async def test_send_morning_weather_sends_formatted_message() -> None:
    ...
    with (
        patch('scheduler.jobs.config.TELEGRAM_GROUP_ID', -1001234567890),
        patch('scheduler.jobs.config.CWA_API_KEY', 'test_cwa_key'),
        patch(
...
def test_setup_scheduler_returns_none_without_required_settings() -> None:
    from scheduler.jobs import setup_scheduler

    bot = MagicMock()

    with (
        patch('scheduler.jobs.config.TELEGRAM_GROUP_ID', None),
        patch('scheduler.jobs.config.CWA_API_KEY', None),
    ):
        scheduler = setup_scheduler(bot)
